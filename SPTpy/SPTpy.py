...
class SlimFastApp:
    ...
    def load_spoton_core(self):
        ...
        def _on_fit():
            ...
            t1 = time.perf_counter()  # <--- 必须在这里重新赋值给 t1
            elapsed = t1 - t0  # <--- 必须紧跟在 t1 赋值之后
    ...
    def load_slimfast(self):
        ...
        menu_fig.title("One-Stop SPT Data Viewer")
...
#####################################################################################
    ...
    def localize_particles(self, image_bin, start_pnt, end_pnt, stack,x0_py,y0_py,x1_py,y1_py):
        ...
        roi_h = y1_py - y0_py
        roi_w = x1_py - x0_py
        if roi_h < wn or roi_w < wn:
            raise ValueError(
                f"ROI size ({roi_w}x{roi_h}) is smaller than the detection "
                f"window ({wn}x{wn}). Please use a larger ROI or reduce the "
                f"detection window size."
            )
    ...
    def build_tracks(self):
        ...
        for t in range(start_point, end_point + 1):
            ...
            if t == start_point:
                ...
                # self.tab_param[:, 0] = np.arange(0, n_init, dtype=float)
                self.tab_param[:n_init, 0] = np.arange(0, n_init, dtype=float)
                ...
                self.tab_var[:n_init, col_start:col_start + BLOCK] = block_var
            ...
            if t > start_point and getattr(self, 'tab_param', None) is not None and self.tab_param.size > 0:
                ...
                # --- 优化后的高效内存操作 ---
                if cur_block >= 1:
                    start_idx = blk_col(1, 1)  # 第 2 块的起始列
            ...
            # progress_var.set(t - start_point + 1)
            # progress_win.update_idletasks()
            # # --- 修改为 (增加 check_winfo_exists 保护) ---
            # if progress_win.winfo_exists():
            #     progress_var.set(t - start_point + 1)
            #     try:
            #         progress_win.update_idletasks()
            #     except Exception:
            #         pass  # 如果窗口在这一瞬间被关了，直接忽略报错
    ...
    def rapport_detection(self, traj, t, lest, part_indices, wn):
        """
        向量化重构版本：支持一次计算一个轨迹对多个粒子的匹配概率。

        参数:
        - traj: 轨迹索引 (int)，如果 < 0 则计算新粒子检测概率。
        - t: 当前时间块索引。
        - lest: 检测数据表 (N x 7 array)。
        - part_indices: 粒子索引，可以是单个 int，也可以是 list/array。
        - wn: 检测窗口大小。

        返回:
        - scores: numpy 数组（每个粒子的对数似然得分）。
        - flag_full_alpha: 针对第一个粒子的状态标记（兼容旧逻辑）。
        """
        # --- 1. 预提取局部变量，减少 self 访问开销 ---
        poids_alpha = self.image_bin['intLawWeight']
        poids_diff = self.image_bin['diffLawWeight']
        t_off = self.T_off
        im_t = self.im_t
        n_pixels = wn * wn

        # 强制将 part_indices 转为 numpy 数组以便向量化
        part_indices = np.atleast_1d(part_indices).astype(int)
        ...
        if traj < 0:
            ...
            for p in part_indices:
                sig2_h1 = max(1e-10, lest[p, 4])
                pi, pj = int(round(lest[p, 1])), int(round(lest[p, 2]))

                # 快速切片计算方差
                di = slice(max(0, pi - offset), min(h, pi - offset + wn))
        ...
        # A. 计算 Blink 概率 (对该轨迹是常数)
        l_blink = 0.0
        blink_status = self.tab_param[traj, 7 * t + 7]
        if blink_status < 0:
            nb_blink = -blink_status
            sig_blink = -t_off / 3.0
            # 使用 math 标量计算比 numpy 快
            p_blink = (2.0 / (math.sqrt(2 * math.pi) * sig_blink)) * \
                      math.exp(-(nb_blink ** 2) / (2 * sig_blink ** 2))
            l_blink = math.log(max(1e-12, p_blink))

        # B. 计算强度概率 (Lalpha) - 向量化计算所有粒子
        ...
        # 高斯分布概率
        p_alpha_gauss = (1.0 / (math.sqrt(2 * math.pi) * sig_alpha)) * \
                        np.exp(-((alphas - alpha_moy) ** 2) / (2 * sig_alpha ** 2))
        # 均匀分布概率
        p_alpha_univ = np.where(alphas < alpha_max, 1.0 / alpha_max, 0.0)

        l_alpha = np.log(np.maximum(1e-12, poids_alpha * p_alpha_gauss + (1 - poids_alpha) * p_alpha_univ))

        # 获取第一个粒子的 flag
        flag_full_alpha = 1 if p_alpha_gauss[0] > p_alpha_univ[0] else 0

        # C. 计算位置/扩散概率 (Ln0) - 向量化
        i0, j0 = lest[part_indices, 1], lest[part_indices, 2]
        ic, jc = self.tab_param[traj, 7 * t + 2], self.tab_param[traj, 7 * t + 3]

        sig_ij_ref = self.sigij_blink(traj, t)
        sig_free_blk = self.sigij_free_blink(traj, t)

        dist_sq = (i0 - ic) ** 2 + (j0 - jc) ** 2

        p_n_ref = (1.0 / (2 * math.pi * sig_ij_ref ** 2)) * np.exp(-dist_sq / (2 * sig_ij_ref ** 2))
        p_n_free = (1.0 / (2 * math.pi * sig_free_blk ** 2)) * np.exp(-dist_sq / (2 * sig_free_blk ** 2))

        l_n0 = np.log(np.maximum(1e-12, poids_diff * p_n_ref + (1 - poids_diff) * p_n_free))
        ...
        if sig_r_ref > 1e-10:
            l_r = -0.5 * math.log(sig_r_ref) - (1.0 / (2 * sig_r_ref ** 2)) * (rs - r_ref) ** 2
        else:
            l_r = np.zeros_like(rs)

        # 最终汇总得分数组
        out_scores = l_alpha + l_n0 + l_blink + 0 * l_r

        # 如果只传入一个粒子，返回标量（兼容旧版调用逻辑）
        if len(out_scores) == 1:
            return out_scores[0], flag_full_alpha

        return out_scores, flag_full_alpha
